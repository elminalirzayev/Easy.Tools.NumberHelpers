...
namespace Easy.Tools.NumberHelpers.Extensions
{
    ...
    public static class NumberToWordsExtensions
    {
        ...
        private static readonly Dictionary<(string Lang, string Country), (string Currency, string SubCurrency)> CountryCurrencyNames = new()
        {
        ...
        /// <param name="isBankingMode">If true, adds explicit 'one' prefix (e.g., 'bir yüz') for financial security. Default is true.</param>
        ...
        public static string ToWordsCurrency(this decimal amount, string languageCode = "en", string? countryCode = null,
            string? currencyName = null, string? subCurrencyName = null, bool isBankingMode = true)
        {
            ...
            {
                names = DefaultCurrencyNames.ContainsKey("en") ? DefaultCurrencyNames["en"] : ("money", "coins");
            }
            ...
            var wholeWords = whole.ToWords(languageCode, isBankingMode);
            var fractionWords = fraction.ToWords(languageCode, isBankingMode);
        ...
        public static string ToWords(this long number, string lang = "en", bool isBankingMode = true)
        {
            lang = lang.ToLower();
            ...
            if (number < 0)
            {
                ...
                return $"{prefix} {ToWords(-number, lang, isBankingMode)}";
            }

            return ConvertNumberToWords(number, lang, isBankingMode);
        }

        // Overloads to support common integer types seamlessly
        ...
        /// <param name="isBankingMode">Enable banking format (e.g. 'bir yüz').</param>
        /// <returns>String representation.</returns>
        public static string ToWords(this int number, string lang = "en", bool isBankingMode = true) => ((long)number).ToWords(lang, isBankingMode);

        /// <summary>
        /// Converts a short to words.
        /// </summary>
        /// <param name="number">Number to convert.</param>
        /// <param name="lang">Language code.</param>
        /// <param name="isBankingMode">Enable banking format (e.g. 'bir yüz').</param>
        /// <returns>String representation.</returns>
        public static string ToWords(this short number, string lang = "en", bool isBankingMode = true) => ((long)number).ToWords(lang, isBankingMode);

        /// <summary>
        /// Converts a byte to words.
        /// </summary>
        /// <param name="number">Number to convert.</param>
        /// <param name="lang">Language code.</param>
        /// <param name="isBankingMode">Enable banking format (e.g. 'bir yüz').</param>
        /// <returns>String representation.</returns>
        public static string ToWords(this byte number, string lang = "en", bool isBankingMode = true) => ((long)number).ToWords(lang, isBankingMode);

        /// <summary>
        /// Converts a float to words (truncates decimals).
        /// </summary>
        /// <param name="number">Number to convert.</param>
        /// <param name="lang">Language code.</param>
        /// <param name="isBankingMode">Enable banking format (e.g. 'bir yüz').</param>
        /// <returns>String representation.</returns>
        public static string ToWords(this float number, string lang = "en", bool isBankingMode = true) => ((long)number).ToWords(lang, isBankingMode);

        /// <summary>
        /// Converts a double to words (truncates decimals).
        /// </summary>
        /// <param name="number">Number to convert.</param>
        /// <param name="lang">Language code.</param>
        /// <param name="isBankingMode">Enable banking format (e.g. 'bir yüz').</param>
        /// <returns>String representation.</returns>
        public static string ToWords(this double number, string lang = "en", bool isBankingMode = true) => ((long)number).ToWords(lang, isBankingMode);

        /// <summary>
        /// Converts a decimal to words (truncates decimals).
        /// </summary>
        /// <param name="number">Number to convert.</param>
        /// <param name="lang">Language code.</param>
        /// <param name="isBankingMode">Enable banking format (e.g. 'bir yüz').</param>
        /// <returns>String representation.</returns>
        public static string ToWords(this decimal number, string lang = "en", bool isBankingMode = true) => ((long)number).ToWords(lang, isBankingMode);

        private static string ConvertNumberToWords(long number, string lang, bool isBankingMode)
        {
            // 0-20 Range
            if (number < 21)
            {
                return LanguageMaps[lang].TryGetValue((int)number, out var word) ? word : number.ToString();
            }
            ...
            if (number < 100)
            {
                ...
                string separator = (lang == "tr" || lang == "az" || lang == "ru") ? " " : "-";

                return ones == 0 ? map[tens] : $"{map[tens]}{separator}{map[ones]}";
            }

            // Scale handling (Hundreds, Thousands, Millions...)
            foreach (var scale in ScaleNumbers[lang])
            {
                if (number >= scale.Value)
                {
                    ...
                    // TR/AZ İÇİN RESMİ vs NORMAL AYRIMI:
                    // Yüz (100) ve Bin (1000) katları için özel kontrol.
                    if ((lang == "tr" || lang == "az") && scale.Value <= 1000 && scaleCount == 1)
                    {
                        // Banking Mode: Açıkça "Bir Yüz", "Bir Bin" yaz.
                        // Normal Mode: Sadece "Yüz", "Bin" yaz.
                        if (isBankingMode)
                        {
                            prefix = $"{ConvertNumberToWords(scaleCount, lang, isBankingMode)} {scaleWord}";
                        }
                        else
                        {
                            prefix = scaleWord;
                    ...
                    {
                        // Diğer diller veya 1'den büyük katsayılar (örn: İki Yüz)
                        prefix = $"{ConvertNumberToWords(scaleCount, lang, isBankingMode)} {scaleWord}";
                    }
                    ...
                    return $"{prefix} {ConvertNumberToWords(remainder, lang, isBankingMode)}";
                }
            }
